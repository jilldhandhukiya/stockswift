...
export default function ReceiptsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-300 selection:bg-indigo-500/30">
            ...
            <table className="min-w-full table-fixed">
              ...
              <tbody>
                {filteredRows.map((r, i) => (
                  <tr
                    key={r.reference}
                    className={`transition-all duration-200 ${i % 2 === 0 ? 'bg-slate-900/10' : ''} hover:bg-indigo-950/70 hover:shadow-lg cursor-pointer group`}
                    onClick={() => router.push(`/receipts/${r.reference}`)}
                  >
                    <td className="px-4 py-4 font-semibold text-white whitespace-nowrap group-hover:text-indigo-400 transition">{r.reference}</td>
                    <td className="px-4 py-4 text-slate-300 whitespace-nowrap group-hover:text-indigo-300 transition">{r.from}</td>
                    <td className="px-4 py-4 text-slate-300 whitespace-nowrap group-hover:text-indigo-300 transition">{r.to}</td>
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        <Avatar name={r.contact} />
                        <div>
                          <div className="text-sm font-medium text-slate-200 group-hover:text-indigo-200 transition">{r.contact}</div>
                          <div className="text-xs text-slate-500 group-hover:text-indigo-400 transition">{r.from} • {r.to}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-4 text-sm text-slate-400 whitespace-nowrap group-hover:text-indigo-300 transition">
                      <div className="flex items-center">
                        <Calendar className="w-4 h-4 mr-2 text-slate-400 group-hover:text-indigo-400 transition" />
                        <span className="text-slate-200 font-medium group-hover:text-indigo-200 transition">{r.schedule}</span>
                      </div>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="flex items-center justify-end gap-3">
                        ...
                        <div className="relative">
                          <button onClick={(e)=>{ e.stopPropagation(); setOpenMenu(openMenu===r.reference?null:r.reference)}} className="p-2 text-slate-400 hover:text-white rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"><MoreHorizontal className="w-4 h-4" /></button>
                          {openMenu===r.reference && (
                            <div className="absolute right-0 mt-2 w-44 bg-slate-900 border border-slate-800 rounded-lg shadow-2xl z-50 flex flex-col py-2 animate-fade-in">
                              <button onClick={(e)=>{ e.stopPropagation(); router.push(`/receipts/${r.reference}`)}} className="w-full text-left px-4 py-2 text-sm text-slate-300 hover:bg-slate-800 transition">View</button>
                              <button onClick={(e)=>{ e.stopPropagation(); setModalData(r); setModalOpen(true); setOpenMenu(null);}} className="w-full text-left px-4 py-2 text-sm text-slate-300 hover:bg-slate-800 transition">Edit</button>
                              <button onClick={(e)=>{ e.stopPropagation(); handleDelete(r.reference)}} className="w-full text-left px-4 py-2 text-sm text-rose-400 hover:bg-slate-800 transition">Delete</button>
                            </div>
                          )}
                        </div>
                      </div>
      ...
      {/* Receipt edit/create modal (reused) */}
      {modalOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-slate-900 border-b border-slate-700 px-6 py-4 flex justify-between items-center">
              ...
              <button onClick={()=>setModalOpen(false)} className="text-slate-400 hover:text-white p-1 rounded"><svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg></button>
            </div>

            <form onSubmit={(e)=>{ e.preventDefault(); setModalOpen(false); }} className="p-6 space-y-6">
              <div>
                <label className="block text-sm font-semibold text-slate-300 mb-2">Reference</label>
                <input defaultValue={modalData?.reference} className="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-300 mb-2">Receive From</label>
                  <input defaultValue={modalData?.from} className="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white" />
                </div>

                <div>
                  <label className="block text-sm font-semibold text-slate-300 mb-2">Schedule Date</label>
                  <input type="date" defaultValue={modalData?.schedule} className="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-semibold text-slate-300 mb-2">Contact</label>
                <input defaultValue={modalData?.contact} className="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white" />
              </div>

              <div>
                <label className="block text-sm font-semibold text-slate-300 mb-2">Products</label>
                <textarea defaultValue={modalData?.product || ''} className="w-full min-h-[80px] px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white" />
              </div>

              <div className="flex gap-3 pt-4 border-t border-slate-700">
                <button type="button" onClick={()=>setModalOpen(false)} className="flex-1 px-4 py-2.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300">Cancel</button>
                <button type="submit" className="flex-1 px-4 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white">Save</button>
              </div>
            </form>
          </div>
        </div>
