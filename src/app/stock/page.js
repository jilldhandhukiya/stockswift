    ...
    <div className="min-h-screen bg-[#020617] text-slate-300 font-sans selection:bg-indigo-500/30">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white mb-2">{t('currentStockTitle')}</h1>
            <p className="text-slate-500 text-sm">{t('currentStockSubtitle')}</p>
          </div>
          
          {/* Quick Stats Summary */}
          <div className="flex gap-3">
            <StatBadge value="₹ 2.4M" label={t('totalValue')} color="bg-emerald-900/20 border-emerald-800 text-emerald-400" />
            <StatBadge value="2 Items" label={t('lowStockItems')} color="bg-rose-900/20 border-rose-800 text-rose-400" />
          </div>
        </div>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-t-xl p-4 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => { setCategoryFilter(''); setSearchTerm(''); }} className="px-3 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs rounded-lg">{t('reset')}</button>
            </div>
        ...
        {createOpen && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
            <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-xl p-6 relative">
              ...
              <h2 className="text-lg font-bold text-white mb-4">{t('newProduct')}</h2>
              ...
              <form onSubmit={submitCreate} className="space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <button type="button" onClick={() => setCreateOpen(false)} className="px-4 py-2 text-sm bg-slate-800 border border-slate-700 rounded hover:bg-slate-700">{t('cancel')}</button>
